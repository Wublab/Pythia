...
def run_predict(torch_model, dataloader):
    with torch.no_grad():
        ...
        for batch in tqdm(dataloader):
            ...
            w_y_hat, _ = torch_model(wild_feat.transpose(1,0).to(device), edge.transpose(1,0).to(device))
            m_y_hat, _ = torch_model(mut_feat.transpose(1,0).to(device), edge.transpose(1,0).to(device))
            y_hat = torch.log(((nn.Softmax(dim=-1)(m_y_hat))*mut_feat.to(device).transpose(1,0)[0,:,:21]).sum(-1).unsqueeze(-1)) - \
                torch.log(((nn.Softmax(dim=-1)(w_y_hat))*wild_feat.to(device).transpose(1,0)[0,:,:21]).sum(-1).unsqueeze(-1))
    ...
    if test_s669:
        ...
        for pdb in tqdm(data_df['pdb'].unique()):
            ...
            for index, row in sub_df.iterrows():
                ...
                names.append(f"{row['pdb']}_{row['wildtype']}_{seq_index+1}_{row['mutation']}")
                ddgs.append(row['ddg'])
